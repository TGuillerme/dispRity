...
However, because of the infrastructure of the package (especially the on of the \dispRity objects -- see the \href{https://github.com/TGuillerme/dispRity}{other vignettes}), the \texttt{metric} argument has to follow certain slightly restrictive rules described in detail below.
\begin{enumerate}
    \item \texttt{metric} must be between one or three \texttt{function} objects
    \item \texttt{metric} must can have only one argument as input: a \texttt{matrix} or a \texttt{vector}
    \item \texttt{metric} individual functions must be classified in one of the three levels (see below)
    \item \texttt{metric} one of the functions must be a level 1 function (see below)
\end{enumerate}
...
## functions as they reduce the matrix to a single dimension (value):
mean(dummy_matrix)
median(dummy_matrix)
@ 

\subsection{A level 2 function}
A level 2 function will decompose the \texttt{matrix} into a \texttt{vector}.
Several level 2 functions are implemented in \dispRity (see \texttt{?dispRity.metric}) such as the \texttt{variances} function that calculates the variance of each column of the ordinated matrix (i.e. it's axis or dimensions):
...
\begin{enumerate}
  \item if your function can deal with a \texttt{matrix} or a \texttt{vector} as an \texttt{input}.
  \item which level is your function (level 1, 2 or 3, see above).
