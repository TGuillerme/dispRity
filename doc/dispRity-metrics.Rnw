...
However, because of the infrastructure of the package (especially the one of the \dispRity objects -- see the \href{https://github.com/TGuillerme/dispRity}{the \dispRity manual}), the \texttt{metric} argument has to follow certain rules:
\begin{enumerate}
