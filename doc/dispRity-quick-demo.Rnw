...
\title{\dispRity quick demo}

\author{Thomas Guillerme\\guillert@tcd.ie}

\maketitle

This is quick demo aiming to give a quick overview of the \dispRity package (v.\Sexpr{packageVersion("dispRity")}).
...
To keep it short, this package allows to summarise ordinated matrices (e.g. MDS, PCA, PCO, PCoA) into values called \textbf{disparity}. It then allows to visualise and analyse the disparity of the ordinated matrices.

This demo, which will use dummy data will quickly cover the following points:
\begin{enumerate}
\item Installing \dispRity
\item Ordinated matrices
\item Creating submatrices
...
\section{Ordinated matrices}
Ordinated matrices are transformed distance matrices based on various methods (e.g. MDS, PCA, PCO, PCoA). For this example, we will just generate \textit{dummy} data with 26 elements and 15 variables.
...
## This is a multivariate matrix:
dummy_data <- replicate(15, rnorm(26, mean=sample(1:10)))
rownames(dummy_data) <- LETTERS
head(dummy_data[,1:5])
## We can the calculate the distance matrix:
dummy_dist <- dist(dummy_data)
## And ordinate it into 10 dimensions:
dummy_ord <- cmdscale(dummy_dist, k=10)
head(dummy_ord)
@ 
This ordinated data will constitute our multidimensional space (e.g. the ecospace, morphospace, cladistospace, etc...).

\section{Creating submatrices}
We can now create two different groups if we are interested in looking at the difference in occupancy of the multidimensional space between the vowels and the consonants.

<<cust.data>>=
## Creating a data.frame that contains information on the letter type
factors <- data.frame("type"=rep("consonant", 26), row.names=LETTERS,
    stringsAsFactors=FALSE)
factors[c(1,5,9,15,21,25),1] <- rep("vowels", 6)

## Splitting the data into sub-matrices according the the factors data.frame
data_series <- cust.series(dummy_ord, factors)
@ 

Note that from now on, objects are attributed the class \dispRity that summarises the object content.
...
We can now bootstrap to get rid of problems from outliers and also rarefy the dataset to have the same number of vowels or consonants.

<<boot.data>>=
## Simple bootstrapping
boot_data <- boot.matrix(data_series)

## Bootstrapping and rarefaction to get the same number of elements
## in both categories (vowels and consonants)
rare_data <- boot.matrix(data_series, rarefaction=6)

## Again both objects can be quickly summarised by just calling them
boot_data
rare_data
@ 

\section{Calculating disparity}
We can calculate disparity as the sum of the variance of each dimension for each sub-matrix to measure the spread of the data for vowels of consonants.

<<disparity>>=
## Calculating disparity for both objects
disparity_boot <- dispRity(boot_data, metric=c(sum, variances))
disparity_rare <- dispRity(rare_data, metric=c(sum, variances))

## Again, printing the objects will just give basic information.
disparity_boot

## To actually get the results, we can use the summary function
summary(disparity_boot)
summary(disparity_rare)
@ 

\section{Plotting the results}
As we can guess from the summary, our results differ between consonants and vowels but consonants seems not affected by sample size.
But let's visualise it:

<<plot1, fig.width=10, fig.height=5, out.width='1\\linewidth'>>=
## Graphical options
...
plot(disparity_boot, main="raw data", elements=TRUE, ylab=c("disparity", "letters"))
## The disparity-through-time data in a time-sliced ways
plot(disparity_rare, main="rarefied data", elements=TRUE, ylab=c("disparity", "letters"))
@

\section{Testing differences}
Finally, to be sure if the disparity is different between vowels and consonants or not, we can apply a statistical test to both categories:
...
You can use the function \texttt{tree.age} provided in this package but not described in this demo for generating the \texttt{\$root.time} automatically (see \texttt{?tree.age}).
...
See the \href{https://github.com/TGuillerme/dispRity}{other dispRity demos} for other specific examples.
...
When using this method, the oldest boundary of the first bin (or the first slice, see below) is automatically generated as being the root age + 1\% of the the tree length as long as at least three elements are present at that point in time (an extra 1\% is added until reaching the required minimum of three elements).
...
The default status is \texttt{FALSE} but it can be set to \texttt{TRUE} to fully rarefy the data (i.e. remove $n$ taxa for the number of pseudo-replicates, where $n$ varies from the maximum number of taxa present in the dataset to a minimum of 3 elements).
