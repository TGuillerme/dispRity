...
Note that we use the \texttt{master} branch here which is version \Sexpr{packageVersion("dispRity")}.
This is the piping-hot latest version.
If you want a more stable (and tested version), change \texttt{ref = "master"} to \texttt{ref = "release"}.
