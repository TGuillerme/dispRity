...
```{r, fig.width=6, fig.height=6}
## Splitting the data into time subsamples
sliced_data <- time.subsamples(BeckLee_mat99, BeckLee_tree, method = "continuous",
...
One of the first steps in disparity analysis is to split the ordinated space into sub-samples of this space (hereafter called *subsamples*, see [glossary](#glossary)).
These subsamples correspond to regions of the ordinated space that we want to compare to each other.
...
`custom.subsamples` # NC: I'd have called this custom.subsamples. It's only 2 more letters and makes it much more readable, and less about custard.
-------------

The `custom.subsamples` function allows to create these subsamples according to one or more factors determined by the user.
...
We can then feed these lists of species to the `custom.subsamples` function after transforming it into a factorial table:
<!-- # NC: This is a fairly horrible set up to go through! Can't you make a function? -->

```{r}
## Creating the factor table
subsamples_factors <- as.data.frame(
    matrix(data = c(rep("crown", length(crown)), rep("stem", length(stem))),
    ncol = 1, dimnames = list(c(crown, stem), "Group")))
head(subsamples_factors)

## Creating the two subsamples
crown_stem <- custom.subsamples(BeckLee_mat50, subsamples_factors)
```
...
The two subsamples represent two groups occupying specific regions of the mammalian morphospace.
The `dispRity` object itself always contains the original morphospace (`$matrix`) as well as information on which taxa compose the subsamples (`$subsamples`):
...
## And "subsamples" the elements from this ordinated space into the two sub-spaces
str(crown_stem$subsamples)
```
...
```{r}
## Adding all mammals to the a new subsamples (along with crown and stem as before)
subsamples_factors2 <- as.data.frame(
    matrix(data = c(rep("crown", length(crown)), rep("stem", length(stem)),
    rep("Mammalia", Ntip(BeckLee_tree))), ncol = 2,
    dimnames = list(c(crown, stem), c("Group", "Class")), byrow = FALSE))

## Creating the three subsamples: Group.stem/crown and Class.Mammalia
custom.subsamples(BeckLee_mat50, subsamples_factors2)
```

`time.subsamples`  <a name="time.subsamples"></a>
-------------
...
Thus we can calculate the bootstrapped and rarefied disparity in each subsamples of the ordinated space.
...
This function is at the core of `time.subsamples` when using the time slicing method.
It allows to slice a phylogenetic tree at a certain time point and selecting the nodes or tips following the models detailed in the `time.subsamples` [section above](#time.subsamples).
