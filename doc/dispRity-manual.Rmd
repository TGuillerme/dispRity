...
The `make.metric` function implemented in `dispRity` is designed to help testing and finding the dimension-level of the functions.
This function tests:

1.  if your function can deal with a `matrix` or a `vector` as an input;
2.  which is your function's dimension-level according to it's output (dimension-level 1, 2 or 3, see above);
...
```{r}
## Which dimension-level is the mean function? And is able to be used in the dispRity?
make.metric(mean)
