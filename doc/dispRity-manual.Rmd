...
If you need help about the package, hopefully the following manual will be useful.
...
This function simply take procrustes data and performs an ordination:
...
Additionally you can provide to the function a `geomorph.data.frame` object.
...
## Ordinating this data and making a disparity object
...
Of course any other methods for creating the ordination matrix is totally valid - or not use any ordination at all!
The only requirements for the `dispRity` functions is to use matrices with elements as rows and dimensions as columns...

## Simple analysis

Two functions allow to run pipelined analysis simply using an ordination matrix: either calculating the disparity through time or the disparity per groups.

### Example data

For illustrating the following functions, we will use data from @beckancient2014.
This dataset contains an ordinated matrix from 50 mammals' discrete characters (called `BeckLee_mat50`), another matrix of the same 50 mammals and the estimate discrete data from their descendants (thus 50 + 49 rows, called `BeckLee_mat99`), a data frame containing the ages of each taxa in the dataset (called `BeckLee_ages`) and finally a phylogenetic tree with the relations between the 50 mammals (called `BeckLee_tree`).
...
For disparity through time analysis, you will need:
  ...
  * Your very own definition of disparity (here the sum of variances)

In the following example, the matrix and the tree comes from the [previous sections](#example-data).
...
When displayed these `dispRity` objects provides us we information on the operations done to the matrix:
...
Indeed, we asked for three subsamples (evenly spread across the age of the tree), the data was bootstrapped 100 times (default) and the metric used was the sum of variances.
...
## Note that anova assumptions where not checked here!
```
...
We can display information about both groups by simply calling the output variable and then summarising the data (and plotting it!) or/and performing a statistical test to compare both groups (here a Wilcoxon test).
...
This following section contains specific information relative to some functions.
If one of your questions are not covered in these sections, please refer to the function manuals in R or send me an email (<guillert@tcd.ie>).
The several tutorials below describe specific functionalities of some specific functions, please always refer to the function manuals for the full function documentation!

## Time slicing

The function `time.subsamples` allows to divide the ordinated matrix into different time subsamples given a dated phylogeny that contains all the elements (i.e. taxa) from the matrix.
...
It is totally possible that some taxa could have had longer longevity and that it spans across multiple time bins.
...
When using this method, the oldest boundary of the first bin (or the first slice, see below) is automatically generated as being the root age plus 1% of the tree length as long as at least three elements are present at that point in time.
...
This model argument is used when the time slice occurs along a branch of the tree rather than on a tip or a node.
...
```{r, eval=TRUE}
## Generating four time slices every 40 million years under gradual evolution
...
One important step in analysing ordinated matrices is to pseudo-replicate the data to see how robust the results of future analysis are sensitive to outliers in the dataset.
This can be achieved using the function `boot.matrix` to bootstrap or/and rarefy the data.
The default options will bootstrapping the matrix 100 times without rarefaction using the "full" bootstrap method (see below):
...
Rarefaction allows to limit the number of elements to be drawn at each bootstrap replication.
For example, one might be interested in looking at the effect of reducing the number of elements on the results of an analysis.
...
The default argument is `FALSE` but it can be set to `TRUE` to fully rarefy the data (i.e. remove *x* elements for the number of pseudo-replicates, where *x* varies from the maximum number of elements present in each subsamples to a minimum of 3 elements).
...
2.  the(se) function(s) must intake as a first argument a `matrix` or a `vector`;
3.  the(se) function(s) must be of one of the three levels described below;
4.  at least one of the functions must be of level 1 or 2 (see below).

### The function levels

The metric functions levels determine the "dimensionality" level of decomposition of the input ordinated matrix.
...
## Example of level 1 functions
mean(dummy_matrix)
median(dummy_matrix)
```

Any summary metric such as mean or median are good examples of level 1 functions as they reduce the matrix to a single dimension (i.e. one value).
...
Several level 2 functions are implemented in `dispRity` (see `?dispRity.metric`) such as the `variances` or `ranges` functions that calculates the variance or the range of each dimension of the ordinated matrix respectively.
...
## A level 3 metric with a forced matrix output
as.matrix(dist(dummy_matrix))
```

### `make.metric`

Of course, functions can be more complex and involve multiple operations such as the `centroids` function (see `?dispRity.metric`) that calculates the euclidean distance between each elements and the a position in the ordinated space.
...
A silent version of the function is also available to be included in your code.
This can be done using the option `silent = TRUE` and will simply output the level of the metric.

```{r}
## Testing whether mean is level 1
if(make.metric(mean, silent = TRUE) != "level1") {
    message("The metric is not level 1.")
}
## Testing whether var is level 1
...
1: This function uses an estimation of the eigen value that only works for MDS or PCoA ordinations (not PCA).
...
This is possible in the `dispRity` package by calculating disparity as a level 2 metric only!
...
The resulting disparity object being a level 2, it can easily also be transformed into a level 1 one by for example measuring the median distance of all these distributions:
...
It takes as arguments the number of elements (data points) and dimensions to create the space but also allows more fine tuning data simulation: it is possible to give a specific distribution to each dimensions, provide a correlation matrix to link the dimensions or even providing an *a priori* distribution of the variance per distributions!
