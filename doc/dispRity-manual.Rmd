...
Because this package is aimed for multidisciplinary purpose, some names or terms used in this tutorial might be non-familiar to certain fields.
Here is a list of what are the exact meaning of these term:

-   **Ordinated space**: designates the mathematical multidimensional object that will be analysed with this package.
...
-   **Elements**: designates the rows of the ordinated space, elements can be either taxa, field sites, countries, etc...

-   **Dimensions**: designates the columns of the ordinated space. The dimensions can also be referred to as axis or principal components in the case of the ordinated space obtained from a PCA.

-   **Series**: it designates sub-samples of the ordinated space.
    Basically a series contain the same number of dimensions as the ordinated space but may contain a smaller subset of elements.
...
Note that we use the `release` branch here which is version `0.2`. <!-- add version !-->
For the piping-hot (but potentially full of bugs) version, you can change the argument `ref = release` to `ref = master`.
This package depends mainly on the `ape` package and uses functions of several other (`ade4`, `geometry`, `grDevices`, `hypervolume`, `paleotree`, `snow`, and `RCurl`).

Data
----

In this tutorial we are going to use a subset of the ordinated cladistic data from @beckancient2014 <!-- fix citation -->
that contains 50 taxa (elements) ordinated using their cladistic distance (i.e. the distance between their discrete morphological characters).
...
## That's a more than 72 lines of lists of lists of matrices!
```

Therefore this package proposes a specific class of object called `dispRity` objects.
These objects allow to easily use S3 method functions such as `summary.dispRity` (just called as `summary`; see [summary](#summary) section) or `plot.dispRity` (just called as `plot`; see [plot](#plot) section).
...
Note however, that it is always possible to recall the full object using the argument `all = TRUE` in `print.dispRity`:

```{r, eval=FALSE}
## Displaying the full object
print(median_centroids, all = TRUE)
## But that's more than ~ 2500 lines on a 13 inch laptop screen!
```

The package provides some utilities functions directly associated to the `dispRity` object manipulation such as `sort.dispRity`, `extract.dispRity` or `sort.dispRity` (see the full list in the [`dispRity` utilities](#dispRity.utilities) section).
These functions come in handy for modifying the `dispRity` object without having to go into it's structure!

Modular functions
-----------------

This package aims to be a *modular package*.
Its architecture is designed to be fairly easily modified by users.
...
For example, the `dispRity` function intake a `metric` argument that is directly the function for calculating disparity should.
This argument can take some already implemented functions such as `mean` but it can also intake some completely new ones such as the sum divided by length (even though this example sounds like reinventing the wheel):

```{r}
## Disparity measured as the mean value of the ordinated matrix
summary(dispRity(sliced_data, metric = mean))

## A function for measuring the sum divided by the length
...
In order to do so, we need to separate the tree into two subtrees at the node 71 (see [phylogeny above](#mammal_tree)).
...
## Note that the elements here are indicated as numeric values referring to the row numbers in the
## original matrix
```
...
This function intakes a `dispRity` object plus various options namely the `quantile` values for the confidence intervals levels; the `cent.tend` for the 
central tendency to use for summarising the results and two *visual* options which are whether to recall the `dispRity` options and how much digits are wanted in the results.
By default the 50 and 95 quantiles and the median are calculated but these options can be modified as follows:
...
```{r}
## Summarising the disparity using the 88 quantile and the standard deviation as the central
## tendency. 
summary(disp_time_binsBS, quantile = 88, cent.tend = sd)

## If one happens to forgot what was calculated in the summarised object,
...
plot(disp_crown_stemBS, type = "polygon", rarefaction = 20)
```
...
Some of the core functions from the package have a `parallel` option.
