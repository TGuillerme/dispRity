...
data(BeckLee_tree)
plot(BeckLee_tree, cex = 0.8) ; nodelabels(cex = 0.7) ; axisPhylo(root = 140)
```
...
## That's a more than 5000 lines of output!
```
...
## But that's more than 155000 lines!
```

Some utility functions such as `sort.dispRity`, `get.dispRity` or `extract.dispRity` allows to access or reorder to some specific content of the object:

```{r}
## Reordering the series (from the present to the past)
sort.dispRity(median_centroids, decreasing = TRUE)

## Extracting some specific series from the disparity object
get.dispRity(median_centroids, what = c(1,4))

## Extracting the observed disparity
extract.dispRity(median_centroids, observed = TRUE)
...
This package aims to be a *modular package*, that is, it is designed to be fairly easily modified by users.
...
This argument can take some already implemented functions such as `mean` but it can also intake some completely new and revolutionary ones such as the sum divided by length:
...
## That is a bit like reinventing the wheel...
```
...
In this example, we can split the matrix based on the phylogeny and separate to different groups of taxa: the crown mammals (i.e. all the living mammals and their direct ancestor) and the stem mammals (i.e. all the mammals that have no related living descendants).
...
The results of this function (like many others presented below) output a `dispRity` object:
...
In this example, the taxa where split inside each time-bin according to their age.
...
Once we obtain our different series, it is possible to bootstrap and rarefy it (i.e. pseudo-replicating the data).
...
Note that it can also intake a `numerical` value designating the proportion of axis to keep.
...
In practice, the `dispRity` function intakes one or more functions as a definition of disparity. Several of these functions will be already implemented in other packages (such as `stats::median`, `base::sum`, etc.); some others are implemented in this package (listed in `?dispRity.metric`) and finally some others can be defined by the users.
...
Here we are going to define disparity as being the median distance between each element and each dimensions centroids.
...
For both the `boot.matrix` and the `dispRity` functions, it is possible to increase computational speed in some cases using the `parallel` option (see each function’s manuals).
