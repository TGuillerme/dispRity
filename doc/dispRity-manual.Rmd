...
The package can also easily take data from `geomorph` using the `geomorph.ordination` function.
This function simply take procrustes data an performs an ordination:
...
If the latter contains sorting information (i.e. factors), they can be directly used to make a `dispRity` object!
...
We will see what are these `dispRity` objects in the sections below...
...
If one of your questions are not covered in these sections, please refer to the function manuals in R or send me an email. <!-- Add anchor to contacts -->
...
However, the taxa here are considered like single points in time.
It is totally possible that the taxa in question could have a longer longevity (e.g. if the taxonomic focus is at the genera level or above) and that it spans across multiple time bins.
...
One important step in analysing ordinated matrices is to pseudo-replicate the data to seem how robust the results of future analysis are sensitive to outliers in the dataset.
This can be achieved using the function `boot.matrix` to bootstrap or rarefy the ordinated matrix.
 ...
 *  `full` where the bootstrapping is entirely stochastic (*n* elements are replaced by any elements drawn from the dat);
...
For example, one might be interested in looking at the effect of reducing the number of elements to the results of the analysis.
...
For example, one can look at ellipsoid hyper-volume of the ordinated space
<!-- Cite Donohue -->
, the sum and the product of the ranges and variances
<!-- Cite Wills -->
 or the median position of the elements relative to their centroid.
 <!-- Cite Sive -->
...
Because of its architecture, printing `dispRity` objects only summarise their content but does not print the disparity value measured or associated analysis.
<!-- link to dispRity objects -->
...
test.dispRity(disparity_time_slices, test = wilcox.test, comparisons = "sequential",
...
slice_aov <- test.dispRity(disparity_time_slices, test = aov, comparisons = "all")
slice_aov
...
This value can be of course pseudo-replicated (boostraped) in order to obtain a more confident distribution of this summary metric.
...
plot(disparity_centroids_median, ylab = "Distribution of the medians of all the distances")
