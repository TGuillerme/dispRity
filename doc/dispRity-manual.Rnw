...
The function \texttt{test.dispRity} works in a similar way as the \dispRity function: it intakes a \dispRity object (containing disparity measurements), a \texttt{test} and a \texttt{comparison} argument.
The \texttt{test} argument can be any statistical or non-statistical test to apply to the disparity object.
It can be common test function (e.g. \texttt{t.test::stats}) or some functions implemented in \dispRity (see \texttt{dispRity.test}) or even user defined functions.
The \texttt{comparison} argument must indicate the way the test should be applied to the data:
\begin{itemize}
...
This argument can be empty (no correction applied) or can contain one of the corrections from the \texttt{p.adjust::stats} function (see \texttt{?p.adjust}).
