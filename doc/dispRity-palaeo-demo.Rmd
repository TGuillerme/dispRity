...
This demo aims to give quick overview of the `dispRity` package (v.0.2) for palaeobiology analysis. Please refer to [GitHub page](https://github.com/TGuillerme/dispRity) for other vignettes, namely the [`dispRity` manual](https://github.com/TGuillerme/dispRity/blob/master/doc/dispRity-manual.Rmd) that explains the functions in more detail.
...
phylogenetic tree containing 50 taxa along side with a table containing some first and last occurence data for several taxa.
...
The `dispRity` package provides a `time.series` function that allows to split the morphospace time slices (using `method = continuous`) or into time bins (using `method = discrete`).
...
For details about this object class, please refer to the [`dispRity` manual](https://github.com/TGuillerme/dispRity/blob/master/doc/dispRity-manual.Rmd).
...
Once we obtain our different time series, we can bootstrap and rarefy them (i.e. pseudo-replicating the data). The bootstrapping will allow to make each series more robust to outliers and the rarefaction will allow us to compare slices with the same number of taxa to get rid of eventual sampling problems (i.e. more taxa in one bin than the other).
...
For more info, please refer to the [`dispRity` metric vignette](https://github.com/TGuillerme/dispRity/blob/master/doc/dispRity-metrics.Rmd).
...
For that we can use the `plot` function to plot the `dispRity` objects.
...
Because our data is temporally auto-correlated (i.e. what happens in bin Y depends on what happened in bin X) and was pseudo-replicated (i.e. each bootstrap draw creates non-independent time seriess because they are all based on the same time series), we are going to apply a non-parametric mean comparison: the `wilcox.test`.
