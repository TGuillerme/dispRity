...
The first step in such analysis will be to create different series that will represent sub-samples of the ordinated space (i.e. sub-regions within the *n*-dimensional object).
Each of these series contain a certain number of elements (i.e. a subset of the 40 experimental field plots) that have some attributes in common.
...
Similarly than with `cust.series`, `dispRity` displays a `dispRity` object.
...
```{r}
## Bootstrapping the customised series 100 times
(bootstrapped_data <- boot.matrix(customised_series, bootstraps = 100))

## Bootstrapping the series and using rarefaction (i.e. only re-sampling 17 elements each time)
