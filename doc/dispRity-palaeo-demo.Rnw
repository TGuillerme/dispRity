...
This demo showcases a typical disparity-through-time analyse: we are going to test whether the disparity changed through time in a subset of Eutheria since the last 100 million years.
...
The warning message tells us that some taxa were considered as single points in time because we didn't provide temporal range information for all the taxa: they are therefore considered as unique time points without any occurrence range.

The output object is a \dispRity object.
For details about this object class, please refer to the \href{https://github.com/TGuillerme/dispRity}{the \dispRity manual}.
In brief, \dispRity objects are lists of different elements (i.e. disparity results, morphospace sub-samples, etc...) that display only a summary of the object when calling the object for avoiding jamming the \R console.
...
Once we obtain our different subsamples, we can bootstrap and rarefy them (i.e. pseudo-replicating the data).
The bootstrapping will allow to make each subsample more robust to outliers and the rarefaction will allow us to compare slices with the same number of taxa to get rid of eventual sampling problems (i.e. more taxa in one bin than the other).
...
We can now calculate the disparity within each sub-sample along side with some confidence intervals generated by the pseudo-replication step above (bootstraps/rarefaction).
...
Thus, in this example, the disparity is defined by the multi-dimensional variance of each sub-sample (i.e. the spread of the taxa within the morphospace).
