...
if(!nocov) {
    ...
    expect_equal_round(unname(unlist(disparity)), c(2.8175937, 1.5718191, 1.2262642, 0.3840770, 0.2389399, 0.7011024), 2)
}
...
if(!nocov) {
    ...
    expect_equal(unique(unlist(lapply(disparity, dim))), c(1, 1000))
    disparity <- get.disparity(is_covar)
    expect_equal_round(unname(unlist(disparity))[-c(4,5)], c(25.115014, 11.407162, 9.240426, 25.986941, 27.336217, 10.353848)[-c(4,5)], 3)
}
})