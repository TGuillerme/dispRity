...
# Analytically estimate model input parameters for starting values in likelihood optim function
...
# likelihood-optimisation functions

# ou model
    
optim.ou.ml <- function (p, data.model.test, time.split=NULL, n.optima=1, fixed.optima=FALSE, optima.fixed.param=NULL)  {
    ...
    if (fixed.optima == FALSE) {
        optima <- p[-c(1:3)]    
           ou.mean.splits <- sapply(1:n.theta, function(x) ou.mean.fun(anc.state, optima[x], alpha, data.model.test$subsamples))
        mean.ou <- c()
        for(x in 1:n.optima) mean.ou <- c(mean.ou, ou.mean.splits[(time.split[x] + 1) : time.split[x + 1] , x])  
    } else {
        optima <- anc.state
        if(n.optima > 1) optima <- c(optima, p[-c(1:3)])
        ou.mean.splits <- sapply(1:n.theta, function(x) ou.mean.fun(anc.state, optima[x], alpha, data.model.test$subsamples))
        mean.ou <- c()
        for(x in 1:n.optima) mean.ou <- c(mean.ou, ou.mean.splits[(time.split[x] + 1) : time.split[x + 1] , x])  
    }
    return(dmnorm(t(data.model.test$central_tendency), mean = mean.ou, varcov = VCV, log = TRUE))
}

model.test.ou <- function (data.model.test, n.optima=1, time.split=NULL, pool.variance = FALSE, cl = list(fnscale = -1), fixed.optima=FALSE) {
    ...
    time.split <- which.min(abs(data.model.test$subsamples - time.split))    
    ...
    if (fixed.optima == FALSE) {
        input_parameters <- c(data.model.test$central_tendency[1], input_parameters / 10, log(2) / half.life, rep(data.model.test$central_tendency[length(data.model.test$central_tendency)], n.optima))
        lower.model <- c(NA, 1e-10, 1e-08, rep(NA, n.optima))
    } else {
        input_parameters <- c(data.model.test$central_tendency[1], input_parameters / 10, log(2) / half.life, rep(data.model.test$central_tendency[length(data.model.test$central_tendency)], n.optima - 1))
           lower.model <- c(NA, 1e-10, 1e-08, rep(NA, n.optima - 1))
    }
    ...
    optimised_model <- optim(input_parameters, fn = optim.ou.ml, control = cl, method = "L-BFGS-B", lower = lower.model, hessian=FALSE, data.model.test=data.model.test, time.split=time.split, n.optima=n.optima, fixed.optima = fixed.optima)
    ...
    if (fixed.optima == FALSE) {
        ...
        optima <- c(ancestral.state, optimised_model$par[-c(1:3)])
    }
    ...
    parameter.return <- c(log.likelihood, ancestral.state, sigma.squared, alpha, optima, sample_size, sample_k, aic, aicc)
    names(parameter.return)    <-    c("log.likelihood", "ancestral.state", "sigma.squared", "alpha", paste0("optima.", 1:n.optima),  "sample.size", "n.parameters", "AIC", "AICc")
    if(n.optima > 1) { 
        n.par <- length(parameter.return)
        parameter.return <- c(parameter.return,  input.time.split)
        names(parameter.return)[-c(1:n.par)] <- paste0("time.split", 1:length(input.time.split))
    }
...
optim.bm.ml <- function (p, data.model.test)  {
    ...
    return(dmnorm(data.model.test$central_tendency, mean = mean.bm, varcov = VCV, log = TRUE))
}

model.test.bm <- function (data.model.test, pool.variance = FALSE, cl = list(fnscale = -1)) {
   ...
   cl$ndeps[cl$ndeps == 0] <- 1e-08
    ...
    optimised_model <- optim(input_parameters, fn = optim.bm.ml, control = cl, method = "L-BFGS-B", lower = c(NA, 1e-6), data.model.test = data.model.test)
    ...
    parameter.return <- c(log.likelihood, ancestral.state, sigma.squared, sample_size, sample_k, aic, aicc)
    names(parameter.return)    <-    c("log.likelihood", "ancestral.state", "sigma.squared", "sample.size", "n.parameters", "AIC", "AICc")
    return(parameter.return)
}

# stasis model

optim.stasis.ml <- function (p, data.model.test, time.split=NULL, n.optima=1) {
    ...
    return(dmnorm(data.model.test$central_tendency, mean = mean.stasis, varcov = VCV, log = TRUE))
}

model.test.stasis <- function (data.model.test, n.optima=1, time.split=NULL, pool.variance = FALSE, cl = list(fnscale = -1)) {
    
    if (pool.variance)  {
        data.model.test <- pooled.variance(data.model.test, rescale.variance = TRUE)
        }
        
    input_optimisation <- stasis.parameters(data.model.test)
    ...
    if (is.null(cl$ndeps)) {
        cl$ndeps <- abs(input_optimisation/10000)
        }
    cl$ndeps[cl$ndeps == 0] <- 1e-08
       
    optimised_model <- optim(input_optimisation, fn =  optim.stasis.ml, control = cl, method = "L-BFGS-B", lower = c(0, rep(NA, n.optima)), data.model.test=data.model.test, time.split=time.split, n.optima=n.optima)
    ...
    parameter.return <- c(log.likelihood, omega, theta, sample_size, sample_k, aic, aicc)
    names(parameter.return)    <-    c("log.likelihood", "omega", paste0("theta.", 1:n.optima), "sample.size", "n.parameters", "AIC", "AICc")
...
optim.eb.ml <- function (p, data.model.test)  {
    ...
    mean.eb <- rep(anc.state, sample.size)
    VCV <- outer(sigma.squared * ((exp(r.rate * time.out) - 1) / r.rate), sigma.squared * ((exp(r.rate * time.out) - 1) / r.rate), FUN=pmin)    
    diag(VCV) <- diag(VCV) + data.model.test$variance / data.model.test$sample_size
    return(dmnorm(data.model.test$central_tendency, mean = mean.eb, varcov = VCV, log = TRUE))
}

model.test.eb <- function (data.model.test, pool.variance = FALSE, cl = list(fnscale = -1)) {
   ...
   if (input_parameters[2] <= 0) {
    ...
    optimised_model <- optim(input_parameters, fn = optim.eb.ml, control = cl, method = "L-BFGS-B", lower = c(NA, 0, -20), upper=c(Inf, Inf, -1e-6), data.model.test = data.model.test)
    ...
    parameter.return <- c(log.likelihood, ancestral.state, sigma.squared, eb.rate, sample_size, sample_k, aic, aicc)
    names(parameter.return)    <-    c("log.likelihood", "ancestral.state", "sigma.squared", "eb.rate", "sample.size", "n.parameters", "AIC", "AICc")
    return(parameter.return)
}
   
# trend model

optim.trend.ml <- function (p, data.model.test)  {
    ...
    VCV <- sigma.squared * outer(data.model.test$subsamples, data.model.test$subsamples, FUN = pmin)
    diag(VCV) <- diag(VCV) + data.model.test$variance / data.model.test$sample_size
    mean.trend <- rep(anc.state, sample.size) + trend.param * data.model.test$subsamples
    return(dmnorm(data.model.test$central_tendency, mean = mean.trend, varcov = VCV, log = TRUE))
}

model.test.trend <- function (data.model.test, pool.variance = FALSE, cl = list(fnscale = -1)) {
    ...
    optimised_model <- optim(input_parameters, fn = optim.trend.ml, control = cl, method = "L-BFGS-B", lower = c(NA, 0, -100), data.model.test = data.model.test)
    ...
    parameter.return <- c(log.likelihood, ancestral.state, sigma.squared, trend, sample_size, sample_k, aic, aicc)
    names(parameter.return)    <-    c("log.likelihood", "ancestral.state", "sigma.squared", "trend", "sample.size", "n.parameters", "AIC", "AICc")
    return(parameter.return)
}
