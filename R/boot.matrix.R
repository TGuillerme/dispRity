...
boot.matrix <- function(data, bootstraps = 100, rarefaction = FALSE, dimensions, verbose = FALSE, boot.type = "full", prob) {
    ...
    if(class(data) != "dispRity") {
        ...
        if(is.null(data_names)) {
            stop(paste(as.expression(match_call$data), "must be either a matrix or an output from the chrono.subsets or custom.subsets functions."), call. = FALSE)
        } else {
    ...
    if(!missing(prob)) {
        if(probabilistic_subsets) {
            stop(paste0(as.expression(match_call$data), " was generated using a gradual time-slicing (", data$call$subsets[2], ").\nThe prob option is not yet implemented for this case."), call. = FALSE)
            #warning(paste0(match_call$data, " was generated using a gradual time-slicing (", data$call$subsets[2], ").\nThe probabilities generated by chrono.subsets() are multiplied by prob." ))
        } else {
            ...
            if(is.null(prob_names)) {
                stop("prob argument must have names (vector) or dimnames (matrix) attributes.", call. = FALSE)
            } else {
            ...
            if(class_prob == "matrix") {
                ...
                if(length(prob) !=  length(prob_names)) {
                    stop("prob argument must be a matrix with one column only.", call. = FALSE)
                } else {
            ...
            if(!all(prob_names %in% rownames(data$matrix))) {
                stop(paste0("prob argument contains elements not present in ", as.expression(match_call$data), "."), call. = FALSE)
            } else {
