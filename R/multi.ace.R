...
multi.ace <- function(data, tree, models = "ER", threshold = TRUE, special.tokens, special.behaviours, brlen.multiplier, verbose = FALSE, parallel = FALSE, output, castor.options) {
    ...
    warning("add parallel option")

    ## Output a matrix
    make.matrix <- function(results) {#, node_labels) {
        # results <- lapply(results, function(data) do.call(cbind, data))
        # return(mapply(function(result, names) {rownames(result) <- names; return(result)}, results, node_labels, SIMPLIFY = FALSE))
