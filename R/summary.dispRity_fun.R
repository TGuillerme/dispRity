...
lapply.summary <- function(disparity_series, cent.tend, quantiles) {
    ...
    # Apply wrapping function for summarising a single rarefaction
    apply.summary <- function(disparity_series_rare, cent.tend, quantiles) {
        output <- list()
    ...
    return(lapply(disparity_series[-1], apply.summary, cent.tend, quantiles))
}

## Lapply wrapper for fetching elements
lapply.fetch.elements <- function(series, data) {
    return(length(fetch.elements(data, series)))
}

## Lapply wrapper for getting the disparity observed values
lapply.observed <- function(disparity) {
    return(disparity$elements)
}
