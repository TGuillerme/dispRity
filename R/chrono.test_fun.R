...
## started function to take draws from itsa envelope but is not correct way i think
# itsa.sig <- function(itsa) {
#     lapply(itsa$test.output, lapply, function(x){
#         cf_draws_list <- matrix(nrow = nrow(x$data$counter_lower_ci), ncol = 100)
#         for (i in 1:nrow(counter_lower)) {
#             cf_draws_list[i,] <- runif(100, x$data$counter_lower_ci[i,], x$data$counter_upper_ci[i,]) ## get 100 values in between the 95%ci
#         }
#         bind <- list(disparity = cf_draws_list, time_elapsed = x$data$time_elapsed, impact = )

#         for (i in 1:ncol(cf_draws_list)) {
#             lm(cf_draws_list[,i] ~ )
#         }
