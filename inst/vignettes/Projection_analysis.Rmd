...
With the variance-covariance matrices obtained from the `mcmcmcglmmm` analyses, we can use linear algebra to make a mathematical interpretation of @endler2005's elaboration and innovation (here innovation) patterns (Fig 1. in @endler2005).
...
Then, if species evolve parallel to this axis, they are elaborating on traits combinations; conversely, if species evolve orthogonaly to this axis, they are innovating new traits combinations (innovating).
...
In practice, we can use these projections and rejections of vectors or points onto a specific other vector as a way to measure the elaboration and innovation species within a group or of entire groups.
The first instance, measuring the elaboration and innovation of species within a group, is rather straightforward from [Figure 2](#fig2): the idea is to project each points (defined as the dashed vectors in [Figure 2](#fig21)) onto a vector of interest (e.g. either the group's own main axis of variance-covariance or the entire phylogeny's one).

We measured the innovation and elaboration using two main approaches (see [Figure 2](#fig2)):

 * By clades (i.e innovation/elaboration **between** clades), where we calculated the projections of the major axes of each clade onto the global phylogenetic major axes.
 * By species (i.e innovation/elaboration **within** clades), where we calculated the projections of each species onto 1) the global phylogenetic major axes and 2) the phylogenetic major axes of their respective clade.
...
Similarly for innovation, any innovation score comprised in $[0, 1]$ indicates an innovation that is within the major axis' variance-covariance 95% confidence interval; a score of $[1, \infty)$ denotes a particularly explorative species.


# Reproducible example

Here we illustrate the elaboration and innovation analysis pipeline by applying it to a data set from @cooney2017 focused on the birds charadriiformes order.
...
##### Figure 4: {#fig4}
Visualisation of the posteriors of the model: A) the first two dimensions of the inpu trait space; B) the same space with the average ellipses and average major.axis scaled to the size of the phylogenetic (grey) ellipse's size; C) the distribution of 100 random ellipses (unscaled); D) the distribution of 100 random major axes (unscaled).

We can then use this `"dispRity"` object to calculate which of the three charadriiformes orders is the most elaborative and innovating ([Figure 2](#fig2)B) and then the distribution of species absolute ([Figure 2](#fig2)C) and relative ([Figure 2](#fig2)D) elaboration and innovation scores on the 1000 posterior samples.
