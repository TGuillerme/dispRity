...
This vignette provides advanced details on the mini-chains MCMCglmm pipeline and provides code and detail to reproduce the results presented in the paper associated with this vignette.
Due to the amount of data and computational time required to reproduce the results in the paper (2 CPU years and 350GB of data) we only demonstrate the method on the Charadriiformes order.
...
With the variance-covariance matrices obtained from the `mcmcmcglmmm` analyses, we can use linear algebra to make a mathematical interpretation of @endler2005's elaboration and innovation (here exploration) patterns (Fig 1. in @endler2005).
In fact, if we can interpret the variance-covariance matrix as estimated evolutionary space in the trait space that contains all the observed traits of a group and takes evolutionary history into account, we can then use the first eigen vector of that variance-covariance matrix as the major evolutionary axis, i.e. the axis in multidimensional space along which most trait values co-evolve.
In other words, if the variance-covariance matrix is a multidimensional ellipse, it's major axis is its major evolutionary axis.
Then, if species evolve parallel to this axis, they are elaborating on traits combinations; conversely, if species evolve orthogonaly to this axis, they are exploring new traits combinations (innovating).

Mathematically we can use linear algebra to project and reject each species in the trait-space along this major axis of evolution (Figure 2).
...
In practice, we can use these projections and rejections of vectors or points onto a specific other vector as a way to measure the elaboration and exploration species within a group or of entire groups.
The first instance, measuring the elaboration and exploration of species within a group, is rather straightforward from [Figure 3](#fig1): the idea is to project each points (defined as the dashed vectors in figure 2) onto a vector of interest (e.g. either the group's own main axis of variance-covariance or the entire phylogeny's one).

We measured the exploration and elaboration using two main approaches (see [Figure 2](#fig2)):
...
##### Figure 2: {#fig2}
Different types of elaboration exploration analyses for a group of species. A) Given a group of 10 species (in black) with two traits, we can calculate the 95% confidence interval variance-covariance ellipse and its major axis (in blue). Say we also calculated another vector (in orange) that could be for example the major axis of another variance-covariance matrix (e.g. from another group or the whole phylogeny). B) we can calculate the projection of the group's major axis on the other major axis by translating it (black doted arrow) and calculate it's elaboration relative to the orange vector (orange dashed line) or its relative exploration (green line). C) We can also calculate the elaboration and exploration scores for each individual species relative to the orange major axis (e.g. the phylognetic major axis); D) or relative to their own major axis.

<!-- Scaling and interpretation -->
Note that the examples depicted in [Figure 2](#fig2) are not scaled for clarity.
In the algorithm implemented in `dispRity`, the results are always effectively scaled as described in [Figure 3](#fig1) and in the [detailed algorithm](#projections-supplementary).
In practice, this makes the vector onto which elements are projected of length one.
Furthermore, because the projections happen based on the first point of the major axis of interest (which can be arbitrary), we advice to centre the elaboration results on 0.5 and rescaled (effectively removing 0.5 from the scaled projection value and rescaling it to 1) and to make them absolute.
This makes the elaboration and exploration results more intuitive to interpret: an elaboration score comprised in $[0, 1]$ is distributed onto the major axis, a score of $[1, \infty)$ is beyond the major axis (in any _n_ direction).
If the major axis is for example the 95% confidence interval from the phylogenetic variance-covariance matrix, this denotes a species that is especially elaborative.
Similarly for exploration, any exploration score comprised in $[0, 1]$ indicates an exploration that is within the major axis' variance-covariance 95% confidence interval; a score of $[1, \infty)$ denotes a particularly explorative species.
...
We then used these results to understand measure which of the three charadriiformes is the most elaborative and the most exploring.
We tested that both at 1) the entire clade level, by projecting the 1000 posterior major axis of each clade's phylogenetic residuals onto the global phylogeny's residuals; and 2) the species level to see which clade presented the most elaborative and exploring species.
...
covar.plot(covar_data, points = TRUE, points.cex = 0.5, ellipses = mean, major.axes = mean, legend = TRUE, col = c("orange", "blue", "darkgreen", "grey"), apply.to.VCV = TRUE, legend.x = "bottomright")
...
##### Figure 4: {#fig4}
Visualisation of the posteriors of the model: A) the first two dimensions of the inpu trait space; B) the same space with the average ellipses and average major.axis plotted; C) the distribution of 100 random ellipses; D) the distribution of 100 random major axes.")





<!-- TG:TODO: explain this bit -->
...
```{r, echo = FALSE, eval = TRUE}
## Measure the group differences
group_differences <- dispRity.covar.projections(covar_data,
                                                type = "groups",
                                                base = "phylogeny",
                                                output = c("position", "distance"))
## Measure the individual differences
species_differences <- dispRity.covar.projections(covar_data,
                                                  type = "elements",
                                                  base = "phylogeny",
                                                  output = c("position", "distance"))
...
In details, we can define the major axes from the variance-covariance matrices and then project and reject each element of interest in the space onto this axis.
...
Finally, we can centre the matrix $M_{n}$ on the estimated solution of each generalised linear mixed model (`model$Sol`, in `MCMCglmm`) corresponding to the estimation of the position of the variance-covariance matrix in the trait space.
$M_{n}$ then contains all the major axis of the 0.95 hyper-ellipse fitting the variance-covariance matrix.
...
In practice we followed [this procedure](https://math.stackexchange.com/questions/598750/finding-the-rotation-matrix-in-n-dimensions) and applied a modification of [this implementation](https://stackoverflow.com/questions/42520301/find-rotation-matrix-of-one-vector-to-another-using-r/42542385#42542385) (see @aguilera2004 for the formal generalisation of this algorithm in $n$ dimensions) using the following algorithm implemented in `dispRity::projections` (@guillermedisprity):
 ...
 4. Rotate the trait space using the rotation matrix $R_{\vec{B}}$ to satisfy the linear transformation $\vec{B} \rightarrow \vec{\hat{\imath}}$ (with $\vec{\hat{\imath}}$ being the first unit vector of the trait space - typically the x axes unit vector). 
