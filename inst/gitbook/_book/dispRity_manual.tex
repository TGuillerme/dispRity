...
\includegraphics{dispRity_manual_files/figure-latex/unnamed-chunk-52-1.pdf}
...
\includegraphics{dispRity_manual_files/figure-latex/unnamed-chunk-53-1.pdf}
...
\NormalTok{## Beta diversity distance matrix}
\NormalTok{betad <-}\StringTok{ }\KeywordTok{betadiver}\NormalTok{(dune, }\StringTok{"z"}\NormalTok{)}

\NormalTok{## Create a pairwise distance matrix}
\NormalTok{make.pairwise.dist.matrix <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(matrix) \{}
    \ControlFlowTok{if}\NormalTok{(}\KeywordTok{class}\NormalTok{(matrix) }\OperatorTok{==}\StringTok{ "matrix"} \OperatorTok{||}\StringTok{ }\KeywordTok{class}\NormalTok{(matrix) }\OperatorTok{==}\StringTok{ "data.frame"}\NormalTok{) \{}
\NormalTok{        ## Simply calculate the distances}
        \KeywordTok{return}\NormalTok{(}\KeywordTok{dist}\NormalTok{(matrix))}
\NormalTok{    \} }\ControlFlowTok{else}\NormalTok{ \{}
\NormalTok{        ## Transform the distances into a matrix}
\NormalTok{        pair_distances <-}\StringTok{ }\NormalTok{matrix}
\NormalTok{        pairwise_dist <-}\StringTok{ }\KeywordTok{matrix}\NormalTok{(}\OtherTok{NA}\NormalTok{, }\DataTypeTok{nrow =} \KeywordTok{nrow}\NormalTok{(matrix), }\DataTypeTok{ncol =} \KeywordTok{nrow}\NormalTok{(matrix))}
        \KeywordTok{colnames}\NormalTok{(pairwise_dist) <-}\StringTok{ }\KeywordTok{rownames}\NormalTok{(pairwise_dist) <-}\StringTok{ }\KeywordTok{rownames}\NormalTok{(matrix)}
        \KeywordTok{diag}\NormalTok{(pairwise_dist) <-}\StringTok{ }\DecValTok{0}
\NormalTok{        pairwise_dist[}\KeywordTok{lower.tri}\NormalTok{(pairwise_dist)] <-}\StringTok{ }\NormalTok{pair_distances}
\NormalTok{        pairwise_dist <-}\StringTok{ }\KeywordTok{as.dist}\NormalTok{(pairwise_dist)}
\NormalTok{    \}}
\NormalTok{\}}
\NormalTok{pairwise_dist <-}\StringTok{ }\KeywordTok{make.pairwise.dist.matrix}\NormalTok{(dune)}

\NormalTok{## Create the factor group}
\NormalTok{factors <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{factor =}\NormalTok{ dune.env}\OperatorTok{$}\NormalTok{Management)}

\NormalTok{## Testing adonis}
\NormalTok{test1 <-}\StringTok{ }\KeywordTok{adonis}\NormalTok{(betad }\OperatorTok{~}\StringTok{ }\NormalTok{factor, factors)}
\NormalTok{test2 <-}\StringTok{ }\KeywordTok{adonis}\NormalTok{(pairwise_dist }\OperatorTok{~}\StringTok{ }\NormalTok{factor, factors)}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{summary}\NormalTok{(disp_dummy_group)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{summary}\NormalTok{(disp_equal_group)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{## Function for creating the factors table}
\NormalTok{make.factors <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(data) \{}
\NormalTok{    ## Extracting the factors}
\NormalTok{    factors <-}\StringTok{ }\KeywordTok{names}\NormalTok{(}\KeywordTok{sort}\NormalTok{(}\KeywordTok{unlist}\NormalTok{(}\KeywordTok{lapply}\NormalTok{(data}\OperatorTok{$}\NormalTok{subsets, }\ControlFlowTok{function}\NormalTok{(X) }\KeywordTok{return}\NormalTok{(X}\OperatorTok{$}\NormalTok{elements)))))}
\NormalTok{    ## Trimming the factors names}
\NormalTok{    group_names <-}\StringTok{ }\KeywordTok{names}\NormalTok{(data}\OperatorTok{$}\NormalTok{subsets)}
    \ControlFlowTok{for}\NormalTok{(group }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\KeywordTok{length}\NormalTok{(group_names)) \{}
\NormalTok{        factors[}\KeywordTok{grep}\NormalTok{(group_names[group], factors)] <-}\StringTok{ }\NormalTok{group_names[group]}
\NormalTok{    \}}
    \KeywordTok{return}\NormalTok{(}\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{factor =}\NormalTok{ factors, }\DataTypeTok{row.names =} \KeywordTok{rownames}\NormalTok{(data}\OperatorTok{$}\NormalTok{matrix)))}
\NormalTok{\}}

\NormalTok{adonis.dispRity <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(data, ...) \{}
\NormalTok{  ## Create the factor table}
\NormalTok{  factors <-}\StringTok{ }\KeywordTok{make.factors}\NormalTok{(data)  }

\NormalTok{  ## Run adonis}
  \KeywordTok{return}\NormalTok{(}\KeywordTok{adonis}\NormalTok{(}\KeywordTok{make.pairwise.dist.matrix}\NormalTok{(data}\OperatorTok{$}\NormalTok{matrix)}\OperatorTok{~}\NormalTok{factor, factors, ...))}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{dispRity_manual_files/figure-latex/unnamed-chunk-104-1.pdf}
