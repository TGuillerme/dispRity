...
\begin{abstract} 
    ...
    \begin{enumerate}
        ...
        It is then possible to measure divers aspects of the distribution of some observation (e.g. species) in this space.
        ...
        Finally it also contains a suite of utilities to work with \disp objects aimed at helping users to develop their own multidimensional metrics and/or tests.
...
One can then measure how the observations are distributed within this space to answer related questions (e.g. ``does group A occupies more space than group B?'').
...
package maintainers/software developers choose their preferred definition of multidimensional space and disparity metric to best it their needs (i.e. data, hypothesis, etc.) making the implementations sometimes hard to adapt to different needs.
...
\begin{itemize}
    ...
    \item ``random'' where the data chosen along the branch is randomly chosen between the descendant or the ancestor
    \item ``proximity'' where the data chosen along the branch is either the descendant or the ancestor depending on branch length
    \item ``equal.split'' where the data chosen along the branch is both the descendant and the ancestor with an even probability
    \item ``gradual.split'' where the data chosen along the branch is both the descendant and the ancestor with a probability depending on branch length
\end{itemize}
...
In this case one can then test some hypothesis such as: environments with longer altitudinal gradients contain more species.
...
Following the previous example, if disparity is also higher with longer altitudinal gradients, one could argue that the species richness is caused by an increased number of morphologies adapted to more ecological niches (or something akin).
...
The following example is based on the discrete morphological data described above. More examples, including based on other types of data are available in the package vignette.
...
Similarly to the example above, it is also possible to statistically test this hypothesis using, for example, permutational multivariate ANOVA  \citep[PERMANOVA; ][]{permanova} through the \texttt{adonis.disprity} function that is a wrapper of the \texttt{vegan::adonis} function  \citep{oksanen2007vegan} for \texttt{dispRity} objects:
...
Note that is this case, the function calls different warnings on the usage of such test and the eventual data not used in the test.
...
\section{Funding}
This work was funded by the European Research Council under the European Union's Seventh Framework Programme (FP/2007 - 2013)/ERC Grant Agreement number 311092 and the Discovery Project Grant number DP170103227.
