...
\newpage
\begin{abstract} % NC: This needs some work, I think it could be a little longer. Really need to sell the problem the package solves, what questions are asked with it etc. Remember editors ONLY see this initially, so if it is crap it won't go out to review.

\begin{enumerate}
    \item I present \disp, an \texttt{R} package for summarising multidimensional spaces.
    ...
    \item The package provides a flexible framework where the space, metric and test can be easily defined by the user to analyse their specific ecological or evolutionary hypothesis.
    \item Its architecture is based on the \disp object with a variety of tools for modifying, summarising or plotting it.
    \item The package's modularity and simple interface allows users to easily run reproducible multidimensional analyses.
\end{enumerate}
...
The biological variables in such analyses are equally diverse, including morphological traits (discrete traits like the presence or absence of a character, e.g. \citealt{brusattedinosaur2012}; and continuous traits such as lengths, e.g. \citealt{price2015predation}), life history traits \citep[e.g.][]{diaz2016global}, and even ecosystem properties \citep[e.g.][]{DonohueDim}. % NC: Best to avoid etc. where possible
...
See below for a more detailed description of the package features.
...
Additional arguments specific to \disp objects can also be used such as \texttt{observed} to display the observed disparity (i.e. non-bootstrapped) or \texttt{rarefaction} to only plot the disparity for % NC: for...what????
...
Similarly to the \disp function described above, this function can take any test defined by the user or built into base R.
...
\section{Examples} % NC: I think it would be nice to add a short paragraph describing the cool science question you are asking here, why it is interesting etc. Otherwise this is quite a dry software example that doesn't really convince me I need your cool new package.
The following example is based on a classical palaeobiology morphological disparity analysis.
Note more examples are available in the package manual \url{https://rawgit.com/TGuillerme/dispRity/master/inst/gitbook/_book/index.html}.
...
Here, we may be interested in whether the approximation of the volume within the morphospace (the disparity as defined above) is different in living mammals and their descendants (crown) comparaed to the fossil mammals with no living descendants (stem).
These two groups can be defined as follows (where the integers are the row number of the species in the matrix):
...
As we can see from the summary table and the plot, there seems to be a significant difference in morphospace volume occupied between the two groups.
...
% NC: Again a short description of the actual question being asked here and why it is cool.
Another common type of disparity analysis is a disparity-through-time analysis.
Using the same definition of the multidimensional space and disparity as in the previous example, we may be interested in how disparity changes through time between the Late Cretaceous (100.5 to 66.0 million years ago - Mya), the Paleocene (66.0 to 56.0 Mya) and the Eocene (56.0 to 33.9 Mya).
...
\texttt{> plot(disparity, type = 'continuous')}
...
Information concerning each function can be found in R in the individual function manuals and a package manual with more detailed information is available online at \url{https://rawgit.com/TGuillerme/dispRity/master/inst/gitbook/_book/index.html}.
