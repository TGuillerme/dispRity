...
%TODO: link to other clarity changes
...
%Precision up on what is disparity
...
% Define Procrustes variance

Procrustes variance = Sum of the diagonal covariance matrix between each elements divided by the number of elements.
This is appropriate when looking simply at the volume in the multidimensional space. Howver, does not work if we want to look at the distance between two groups or if we want to use another proxy than the procrustes variance for the space occupancy (e.g. the ellipsoid volume might and direction might be more appropriate in XXX case).

%From the manual: Procrustes variance is the same sum of the diagonal elements of the group covariance matrix divided by the number of observations in the group


%From Michael Collyer: As the author indicates, in geomorph we limit our disparity measure to Procrustes variance (same as the disparity metric used in dispRity) and hypothesis tests to use randomization tests that randomly permute residuals (not data) over many permutations to generate pairwise differences in disparity.
