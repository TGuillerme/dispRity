...
// Calculating the MORD character distance
static double R_MORD(double *x, int nr, int nc, int i1, int i2)
{
...
void R_distanceMORD(double *x, int *nr, int *nc, double *d, int *diag, int *method)
{
    int dc, i, j;
...
SEXP C_diff_mord(SEXP x, SEXP smethod, SEXP attrs)
{
    ...
    R_distanceMORD(REAL(x), &nr, &nc, REAL(result), &diag, &method);
