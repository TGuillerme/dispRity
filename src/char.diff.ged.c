...
// Calculating the GED character distance
static double R_GED(double *x, int nr, int nc, int i1, int i2)
{
...
void R_distanceGED(double *x, int *nr, int *nc, double *d, int *diag, int *method)
{
    int dc, i, j;
...
SEXP C_diff_ged(SEXP x, SEXP smethod, SEXP attrs)
{
    ...
    R_distanceGED(REAL(x), &nr, &nc, REAL(result), &diag, &method);
