...
void R_distanceGower(double *x, int *nr, int *nc, double *d, int *diag, int *method)
{
    int dc, i, j;
...
SEXP C_diff_gower(SEXP x, SEXP smethod, SEXP attrs)
{
    // Define the variable
    SEXP result;
    ...
    R_distanceGower(REAL(x), &nr, &nc, REAL(result), &diag, &method);
